...
class Section(Timestamp, Base):
    ...
    title = Column(String(200), nullable=False)
    description = Column(Text, nullable=True)
    course_id = Column(Integer, ForeignKey("courses.id_"), nullable=False)
...
class ContentBlock(Timestamp, Base):
    ...
    title = Column(String(200), nullable=False)
    description = Column(Text, nullable=True)
    type_ = Column(Enum(ContentType))
    url = Column(URLType, nullable=True)
    content = Column(Text, nullable=True)
    section_id = Column(Integer, ForeignKey("sections.id_"), nullable=False)
...
class StudentCourse(Timestamp, Base):
    ...
    id_ = Column(Integer, primary_key=True, index=True)
    student_id = Column(Integer, ForeignKey("users.id_"), nullable=False)
    course_id = Column(Integer, ForeignKey("courses.id_"), nullable=False)
...
class CompletedContentBlock(Timestamp, Base):
    ...
    content_block_id = Column(
        Integer, ForeignKey("content_blocks.id_"), nullable=False
    )
    url = Column(URLType, nullable=True)
